...
export function renderTodoList(container, { pid, project, currentWeek, isFreetime, hasTeam, shared = false }) {
  container.innerHTML = `
    <div>
  ...
  function smartSort(tasks) {
    ...
    return [...tasks].sort((a, b) => {
      const score = t => {
        const thisWeek = ((t.weekSlots??[t.weekSlot].filter(Boolean))[0])?.weekId === currentWeek;
  ...
  function render(tasks) {
    ...
    visible.forEach(task => {
      ...
      el.querySelector('.task-check').addEventListener('click', async () => {
        await updateTask(pid, task.id, { status: 'complete' }, shared);
        toast('Done! ✓', 'success');
      });
