...
export async function renderResourceAudit(container, { pid, shared = false }) {
  container.innerHTML = `
  ...
  const blocks = await getFixedBlocks(pid, shared);
  ...
  container.querySelector('#save-audit').addEventListener('click', async () => {
    ...
    await saveFixedBlocks(pid, blocks, shared);
  ...
  container.querySelector('#clear-audit').addEventListener('click', async () => {
    ...
    await saveFixedBlocks(pid, {}, shared);
    updateFreeCount();
    toast('Cleared');
  });
}
