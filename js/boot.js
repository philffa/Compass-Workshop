...
async function boot() {
  ...
  try {
    ...
    // Step 3: Wait for auth state with recovery for corrupted auth
    await new Promise((resolve, reject) => {
      const timeout = setTimeout(() => {
        reject(new Error('Auth timed out — check your internet connection and refresh'));
      }, 15000);
      ...
      const unsub = onAuthStateChanged(auth, async (user) => {
        ...
        if (!user) {
          try {
            ...
            // Auth internal error — clear state and retry once
            if (e.message?.includes('INTERNAL ASSERTION') || e.code === 'auth/internal-error') {
              try {
                await auth.signOut().catch(() => {});
                // Clear any cached auth state in IndexedDB
                await new Promise(r => setTimeout(r, 500));
                await signInAnonymously(auth);
                resolve();
              } catch(e2) {
                reject(new Error('Sign-in failed — try clearing your browser cache and refreshing'));
              }
            } else {
              reject(e);
            }
        ...
        // onAuthStateChanged itself errored
        clearTimeout(timeout);
