...
function sharedRef(...segs) {
  ...
  // Even number of segments = document, odd = collection
...
export function onProjects(cb) {
  const { onSnapshot, query, orderBy, getDocs, getDoc } = FM();
  ...
  async function fetchAll() {
    ...
    // Shared projects via memberships
    const memberships = await getDocs(userRef('memberships'));
    const sharedList = [];
    for (const m of memberships.docs) {
      const { projectId } = m.data();
      try {
        const pSnap = await getDoc(sharedRef('projects', projectId));
        if (pSnap.exists()) sharedList.push({ ...pSnap.data(), _personal: false });
      } catch(e) { /* project deleted or no access */ }
    }
    cb([...personalList, ...sharedList]);
  }

  // Listen to personal projects changing and refetch everything
  const unsub = onSnapshot(
    query(userRef('projects'), orderBy('createdAt', 'desc')),
    () => fetchAll().catch(console.error)
  );

  // Also trigger once immediately for shared projects
  fetchAll().catch(console.error);
...
export async function getProject(pid) {
  const { getDoc } = FM();
  // Always try personal first — most projects are personal
  try {
    const snap = await getDoc(userRef('projects', pid));
    if (snap.exists()) return { ...snap.data(), _personal: true };
  } catch(e) {
    console.warn('getProject personal lookup failed:', e.code);
  }
  // Only try shared if personal didn't find it
  try {
    const snap = await getDoc(sharedRef('projects', pid));
    if (snap.exists()) return { ...snap.data(), _personal: false };
  } catch(e) {
    console.warn('getProject shared lookup failed:', e.code, '— rules may not be published yet');
    // Return a stub so the app doesn't crash
    return null;
  }
  return null;
}
...
export async function createSharedProject({ name, description, template, icon, templateData }) {
  ...
  const id = genId();
  const ownerId = uid();
  if (!ownerId) throw new Error('Not signed in — please refresh and try again');
  try {
    ...
    console.error('createSharedProject failed at setDoc:', e.code, e.message);
    throw new Error(`Could not create shared project: ${e.message}`);
  }
  try {
    await setDoc(userRef('memberships', id), { projectId: id, role: 'owner', joinedAt: serverTimestamp() });
  } catch(e) {
    console.error('createSharedProject failed at membership:', e.code, e.message);
    throw new Error(`Project created but membership failed: ${e.message}`);
  }
  return id;
}
