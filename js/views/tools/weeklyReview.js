...
export async function renderWeeklyReview(container, { pid, currentWeek, shared = false }) {
  ...
  const { completed, incomplete } = await rollWeekForward(pid, reviewWeek, shared);
  const existing = await getReview(pid, reviewWeek, shared);
  ...
  container.querySelector('#save-review').addEventListener('click', async () => {
    ...
    try {
      ...
      for (const tid of droppedIds) {
        await updateTask(pid, tid, { status: 'archived', weekSlots: [] }, shared);
      }

      // Save review record
      await saveReview(pid, reviewWeek, { reflection, carriedTaskIds: carriedIds }, shared);
