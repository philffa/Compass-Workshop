...
async function generatePDF(pid, project, currentWeek, include) {
  // Fetch all tasks via a one-shot promise from onTasks
  const tasks = await new Promise(resolve => {
    const unsub = onTasks(pid, taskList => {
      unsub();
      resolve(taskList);
    });
  });

  const [weekPlan, fixedBlocks, prevReview] = await Promise.all([
    getWeekPlan(pid, currentWeek),
    getFixedBlocks(pid),
    getReview(pid, prevWeekId(currentWeek)),
  ]);
