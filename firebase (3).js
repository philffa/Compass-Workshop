...
// All Firebase SDK objects come from window globals set by boot.js
// No direct Firebase imports here — avoids CDN/importmap issues in tool files
...
export async function signInAnon() {
  const { signInAnonymously } = window.__firestoreModule
    ? {} : await _authMod();
...
export function onAuth(cb) {
  ...
  cb(auth.currentUser);
  // Then subscribe — auth module already loaded by boot.js
...
function ref(...segs) {
  ...
  // Even number of segments = doc, odd = collection
  return full.length % 2 === 0
    ? doc(db(), ...full)
    : collection(db(), ...full);
}
...
export function onProjects(cb) {
  ...
  return onSnapshot(
    query(ref('projects'), orderBy('createdAt', 'desc')),
...
export function onTasks(pid, cb) {
  const { onSnapshot, query, orderBy } = FM();
  return onSnapshot(
    query(ref('projects', pid, 'tasks'), orderBy('sortOrder')),
    snap => cb(snap.docs.map(d => d.data()))
  );
}
...
export async function updateTask(pid, tid, data) {
  const { updateDoc, serverTimestamp } = FM();
  await updateDoc(ref('projects', pid, 'tasks', tid), {
...
export async function deleteTask(pid, tid) {
  const { deleteDoc } = FM();
  await deleteDoc(ref('projects', pid, 'tasks', tid));
}
...
export async function reorderTasks(pid, orderedIds) {
  ...
  orderedIds.forEach((id, i) =>
    batch.update(ref('projects', pid, 'tasks', id), { sortOrder: (i + 1) * 1000 })
  );
  await batch.commit();
}
...
export async function saveWeekPlan(pid, wid, data) {
  const { setDoc, serverTimestamp } = FM();
  await setDoc(ref('projects', pid, 'plans', wid), {
